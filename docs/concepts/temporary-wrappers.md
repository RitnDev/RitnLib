...
**Exception** : `RitnLibInventory` est un cas particulier — la *table* qu'il reçoit en deuxième argument est par contre persistante (c'est même le but). Voir [Persistance déléguée](delegated-persistence.md) pour le détail.

## Voir aussi

- [Architecture en 4 couches](architecture-layers.md)
- [Modèle d'events](event-model.md)
- [Persistance déléguée](delegated-persistence.md)
