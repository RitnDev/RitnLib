...
- Ce sont des **vues temporaires** : tu en crées une nouvelle à chaque event. **Ne jamais** les stocker dans `storage`. Voir [Wrappers temporaires](concepts/wrappers-temporaires.md) pour le détail.
...
- 🧩 [Architecture en 4 couches](concepts/architecture-couches.md) — comprendre comment RitnLib est organisé.
- ⚠ [Wrappers temporaires](concepts/wrappers-temporaires.md) — la **règle d'or** à connaître avant de toucher aux classes runtime.
